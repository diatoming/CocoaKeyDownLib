...
extension NSEventModifierFlags {
    ...
    /// Returns <true> if the value of the device independent portion of this NSEventModifierFlags
    /// instance matches the flags generated by an arrow key press plus any additional user-specified
    /// flags.
    ///
    /// An arrow key press contains BOTH NSNumericPadKeyMask and NSFunctionKeyMask. This procedure
    /// first checks that both of these flags are present in this instance. If they are, it removes
    /// them, leaving the remaining modifiers. If these outstanding modifiers match the <combination> 
    /// value passed in by the user, the function returns <true>, if not, <false> is returned.
    ///
    /// :param: combination - an NSEventModifierFlags instance, created by OR'ing together individual NSEventModifierFlags options.
    func matchesArrowKeyModifiersInConjunctionWithAdditionalModifiers(combination: NSEventModifierFlags) -> Bool {
        var retval = false
        if containsFunctionKey & containsNumericPadKey {
            let nonArrowFlags = rScreenedFlags - rFunctionKeyMask - rNumericPadKeyMask
            if nonArrowFlags == combination.rawValue {
                retval = true
            }
        }
        return retval
